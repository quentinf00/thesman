...
\section{Evaluation}
We describe so far the choices for developing the methods.
However one critical remaining choice is the criteria that assess the best method to solve a given problem.
To this end, different evaluation data and metrics are considered.
The evaluation data can come from two sources that both comes with their risks and benefits.
The estimated SSH can directly be confronted with observation data.
This method coined Observing System Experiment (OSE), it's main benefit is that it consist the closest setting we have to real world scenario.
However observation data comes with its challenges as well.
Calibrated nadir altimeters are filtered and don't resolve smaller processes.
The same processes that the new SWOT observations are expected to contain.
This creates a real challenge when using nadir altimeter data to evaluate SWOT calibration. Note that observations of other quantities like sea surface temperature or in situ drifter data that are affected by similar processes can be leveraged, but introduce the question of determining the relationship with the estimated SSH.
For altimetry mapping, evaluating on nadir tracks also implies evaluating the SSH on a fraction of the estimated domain. Furthermore the sampling pattern of nadir altimeter limit the evaluation. The irregular acquisition in time prevent confronting the evolution in time of the estimation against a reference. Additionally the one dimensional acquisition do not allow to evaluate relevant geophysical quantities like the vorticity field.
The second source of evaluation data comes from numerical simulations. Dynamical systems with varying level of complexity can be simulated to generate a ground truth SSH field. And similarly observing systems can be simulated to generate pseudo-observations. Both simulations introduce errors with respect to the real world scenario that need to be considered carefully when interpreting the evaluation in such setups. On the other end this evaluation setup provide great flexibility thanks to the available ground truth.

Both scenarii are used in this thesis, Chapter 3 evaluates the SWOT calibration in an OSSE setup while Chapter 4 asks how the evaluation of a neural mapping schemes depends on OSSE and OSE setups.

Multiple metrics are considered in this thesis. The first one is a root mean squared error (RMSE) of the estimated SSH compared with the reference evaluation data. This metric provides a interpretable quantity in centimeter however it misses some considerations. 
The SSH contains processes that have a wide range of scales and amplitudes, in order to better quantify the errors with respect to the amplitude of the SSH, we also use a normalize RMSE (nRMSE) where the previous metric is normalized by root mean squared SSH at each location. More specificcally we use a nRMSE score $mu$ which is $mu = 1 - nRMSE$ (higher score is better).
Furthermore ocean processes of different amplitude have characteristic spatial and temporal scales that are of interest. The more energetic processes have larger spatial scales and longer temporal scales. Domain experts are interested in knowing if certain processes are well represented in the SSH estimation. To this end spectral based metrics are used throughout this thesis. The PSD-score is the one minus the ratio of the power spectrum density (PSD) of the error and the reference SSH. The closer to one this score is for a given scale, the smaller the error signal is relative to the SSH signal at this scale. We further consider the threshold of spatial scales resolved $\lamba_x$ above which the error signal is less than half the SSH signal. Note that in OSE, $\lambda_x$ is computed along the NADIR altimeter track whereas in OSSE, the full field can be used. Furthermore in OSSE, the temporal scales resolved $lambda_t$ can be computed by considering the spatial PSD-score.


On a final note, a non negligeable part of assessing an SSH estimation and deciding on relevant metics rely on qualitatively evaluating at the field, its evolution in time as well as derived geophysical quantities of interest such as geostrophic currents, vorticity. Qualitative evaluation as such requires considerable geophysical knowledge. Chapter 4 is motivated such considerations. It describes and proposes tools on how to implement relevant experimental and evaluation setups to properly develop and evaluate altimetry mapping methods.
